        ...
        <div class="crud-container crud-container-narrow">
            <div class="crud-card">
                <div class="crud-card-body">
                    <form method="POST" action="{{ route('matieres.update', $matiere->id) }}" class="crud-form-grid">
                        @csrf
                        @method('PATCH')

                        <div>
                            <label for="nom_matiere" class="crud-label">Nom</label>
                            <input type="text" name="nom_matiere" id="nom_matiere" class="crud-input" value="{{ old('nom_matiere', $matiere->nom_matiere) }}" required>
                            @error('nom_matiere')
                        ...
                        <div>
                            ...
                            <input type="number" name="volume_heure" id="volume_heure" class="crud-input" value="{{ old('volume_heure', $matiere->volume_heure) }}" min="1" required>
                            @error('volume_heure')
                                <span class="crud-error">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="niveau" id="niveau" class="crud-input" required>
                                <option value="">Sélectionner...</option>
                                <option value="Licence" {{ old('niveau', $matiere->niveau) == 'Licence' ? 'selected' : '' }}>Licence</option>
                                <option value="Master" {{ old('niveau', $matiere->niveau) == 'Master' ? 'selected' : '' }}>Master</option>
                            </select>
                            @error('niveau')
                                <span class="crud-error">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4" class="crud-input">{{ old('description', $matiere->description) }}</textarea>
                        </div>
